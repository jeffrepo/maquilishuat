                ...
                if factura.state == 'paid':
                    ...
                    for linea in factura.invoice_line_ids:
                        if 'Colegiaturas' in linea.name:
                            ...
                            pagadas[grado.id]['alumnos'].apps({'factura_no': factura.number,'matricula': factura.partner_id.matricula,'fecha': factura.date_invoice,'nombre': factura.partner_id.name, 'valor_pagado': factura.amount_total})
                ...
                for linea in factura.invoice_line_ids:
                    ...
                    if 'MENSUAL ANTICIPADO' in linea.name:
                        ...
                        if mes_f == mes_letras:
                            if grado.id not in pagadas:
                                pagadas[grado.id] = {'grado': grado.nombre, 'alumnos': []}
                            pagadas[grado.id]['alumnos'].append({'factura_no': factura.number,'matricula': factura.partner_id.matricula,'fecha': factura.date_invoice,'nombre': factura.partner_id.name, 'valor_pagado': factura.amount_total})
