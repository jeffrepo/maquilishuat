                    ...
                    if cuenta_id.user_type_id.id in tipo['tipo_cuentas']:
                        ...
                        if movimientos:
                            ...
                            if tipo['type'] in ['documentos_por_pagar'] and ('210102' in cuenta_id.code):
                                for movimiento in movimientos:
                                    if (movimiento.payment_id and movimiento.payment_id.invoice_ids and movimiento.payment_id.invoice_ids.out_invoice) or (movimiento.invoice_id and movimiento.invoice_id.type in ['out_invoice','out_refund']):
                                        movimiento_dic = {
                                            "concepto": str(movimiento.ref)+ ' ' + str(movimiento.partner_id.name),
                                            "debe": movimiento.debit,
                                            "haber": movimiento.credit,
